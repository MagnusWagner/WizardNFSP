...
from rlcard.games.wizard import Game
from rlcard.games.wizard.utils import encode_cards, encode_color
from rlcard.games.wizard.utils import ACTION_SPACE, ACTION_LIST
...
class WizardEnv(Env):
    ...
    def __init__(self, config):
        ...
        self.game = Game(num_players = self.config["game_num_players"],num_cards = self.config["game_num_cards"], seed=self.seed)
        super().__init__(config)
        # self.state_shape = [[249+self.config.get("game_num_cards")+5*self.num_players+1] for _ in range(self.num_players)]
        self.state_shape = [[240+5+1+1+4*self.num_players+1] for _ in range(self.num_players)]  
    ...
    def _extract_state(self, state):
        ...
        Extract state and add:
            - "hand": 4x15 matrix of cards in hand
            - "played_cards": 4x15 matrix of cards already played
            - "played_cards_in_trick": 4x15 matrix of cards already played this trick
            - "legal_actions": 4x15 matrix of cards that can be played by agent
        ...
        obs = np.concatenate((obs,color_to_play,num_cards_left_array,num_actions_left_in_trick_array,players_without_color_array,wizard_played))
