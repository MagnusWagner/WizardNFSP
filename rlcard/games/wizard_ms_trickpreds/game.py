...
import pandas as pd
from rlcard.games.wizard_ms_trickpreds import Dealer, Player, Card
from rlcard.games.wizard_ms_trickpreds.utils import COLOR_MAP,COLOR_MAP_WORDS, getPlayerOrders, cards2list, encode_cards, convert_hitscores_to_points
...
class WizardGame:
    ...
    def init_game(self):
        ''' 
        Initialize players and state and a lot more.
        ...
        self.starting_id=np.random.randint(low=0,high=self.num_players)
        self.starting_player = self.players[self.starting_id]
        ...
        for idx, player in enumerate(self.players):
            if self.human_ids and idx in self.human_ids:
                ...
                print("")
                player.tricks_predicted = int(input(f'>> How many tricks do you want to make? (Max. of {self.num_cards})": '))
            else:
