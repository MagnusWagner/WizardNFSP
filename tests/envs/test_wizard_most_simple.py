...
class TestWizardEnv(unittest.TestCase):
    ...
    def test_reset_and_extract_state(self):
        env = rlcard.make('wizard_most_simple')
        state, _ = env.reset()
        self.assertEqual(state['obs'].size, 51) ###

    def test_is_deterministic(self):
        self.assertTrue(is_deterministic('wizard_most_simple'))
    ...
    def test_get_legal_actions(self):
        ...
        for legal_action in legal_actions:
            self.assertLessEqual(legal_action, 12) ###
